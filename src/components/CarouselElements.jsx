...
export default function CarouselElements({ closeModal }) {
  const { state, dispatch } = useContext(Context);
  const { allDirectories, currentDirectory, currentPhoto } = state;
  const [loaded, setLoaded] = useState(false);
  const [direction, setDirection] = useState(0);

  const currentIndex = allDirectories[currentDirectory].findIndex(
  ...
  const changePhotoId = (newIndex) => {
    if (newIndex >= 0 && newIndex < allDirectories[currentDirectory].length) {
      dispatch({
        type: "SET_CURRENT_PHOTO",
        payload: allDirectories[currentDirectory][newIndex],
      });
          ...
          {loaded && (
            <div className="relative aspect-[3/2] max-h-full w-full">
              <>
                ...
                {currentIndex + 1 <
                  allDirectories[currentDirectory]?.length && (
                  <button
          ...
          <div className="fixed inset-x-0 bottom-0 z-40 overflow-hidden bg-gradient-to-b from-black/0 to-black/60">
            ...
            >
              <AnimatePresence initial={false}>
                {allDirectories[currentDirectory].map((photo, index) => (
                  <motion.button
                    key={photo.photoId}
                    initial={{ width: "0%" }}
                    animate={{
